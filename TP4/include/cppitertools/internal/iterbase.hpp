...
namespace iter
{
    namespace impl
    {
        ...
        template< typename T >
        struct IsIterable< T, std::void_t< iterator_type< T > > > :
            std::true_type
        { };
        ...
        using has_random_access_iter =
            is_random_access_iter< iterator_type< T > >;

        //because std::advance assumes a lot and is actually smart, I need a
        //dumb
        ...
        void dumb_advance_impl(Iter&          iter,
                               const EndIter& end,
                               Distance       distance,
                               std::false_type)
        {
        ...
        {
            dumb_advance_impl(
                iter, end, distance, is_random_access_iter< Iter > {});
        }
        ...
        template< typename ForwardIt, typename Distance = std::size_t >
        ForwardIt
        dumb_next(ForwardIt it, const ForwardIt& end, Distance distance = 1)
        {
        ...
        public:
            template< typename T,
                      typename = std::enable_if_t< !is_iterable< T > > >
        ...
        struct IterToolFnOptionalBindFirst :
            PipeableAndBindFirst<
                IterToolFnOptionalBindFirst< ItImpl, DefaultT > >
        {
        private:
            using Base = PipeableAndBindFirst<
                IterToolFnOptionalBindFirst< ItImpl, DefaultT > >;
        ...
        public:
            ...
            ItImpl< T, Container > operator()(T           func,
        ...
        public:
            ...
            ItImpl< Container, T > operator()(Container&& container,
                                              T           func) const
            {
        ...
        template< template< typename > class ItImpl >
        struct IterToolFnBindSizeTSecond
        {  //NOTE not pipeable
        private:
