...
///programme. \author Francois-R.Boyer@PolyMtl.ca \version 2021-12-01 \since
///2020-04
...
namespace bibliotheque_cours
{
    ...
    static constexpr MarqueurVerificationAllocation
        depuisDebutVerificationAllocation = 0;
    ...
    struct InfoBlocMemoire
    {
        ...
        bool repond_aux_criteres(bool seulement_avec_numeros_ligne,
    ...
    {
    public:
        SansVerifierAllocations() : deja_actif(est_actif)
        {
            est_actif = true;
        }

        ~SansVerifierAllocations()
        {
            if (!deja_actif)
                est_actif = false;
        }

        bool etait_deja_actif()
        {
            return deja_actif;
        }
    ...
    bool tous_les_new_ont_un_delete(bool seulement_avec_numeros_ligne = false,
                                    MarqueurVerificationAllocation depuis =
                                        depuisDebutVerificationAllocation);
    void dump_blocs_alloues(bool seulement_avec_numeros_ligne = false,
                            MarqueurVerificationAllocation depuis =
                                depuisDebutVerificationAllocation);
    ...
    ///Devrait être instaciée une seule fois au début du "main" pour vérifier
    ///les fuites du programme (excluant les allocations faites avant
    ///l'exécution du "main").
    ...
    public:
        ...
        //On désactive à la fin du main, pour ne pas qu'un delete hors main
        //donne une erreur comme quoi il n'a pas de new associé, mais on affiche
        //les fuites seulement plus tard.
        ...
        static bool est_dans_phase_apres_main()
        {
            return est_phase_apres_main;
        }

    private:
        struct VerifierALaFin
        {
            ~VerifierALaFin()
            {
                afficher_fuites();
    ...
    void set_breakpoint_sur_allocations(std::size_t* numeros,
                                        std::size_t  nNumeros);
    ...
    public:
        ...
        BreakpointSurAllocations(Ts... numeros) : numeros_ {numeros...}
        {
            set_breakpoint_sur_allocations(numeros_, N);
        }
    ...
    template< typename... Ts >
    BreakpointSurAllocations(Ts...)
        -> BreakpointSurAllocations< sizeof...(Ts) >;

    std::unordered_map< void*, InfoBlocMemoire >&        get_blocs_alloues();
    //Utiliser get_blocs_alloues au lieu de la référence dans les opérateurs
    //new/delete qui pourraient être appelés avant l'initialisation de cette
    //variable.
