...
private:
    ...
    {
        IteratorData() = delete;
        static_assert(std::tuple_size< std::decay_t< TupTypeT > >::value
                          == sizeof...(Is),
                      "tuple size != sizeof Is");

        static_assert(are_same< iterator_deref<
                          std::tuple_element_t< Is, TupTypeT > >... >::value,
    ...
    public:
        ...
        template< std::size_t Idx >
        static bool get_and_check_not_equal(const IterTupType& lhs,
                                            const IterTupType& rhs)
        {
...
public:
    ...
    public:
        ...
        Iterator(std::size_t                      i,
                 typename IterData::IterTupType&& iters,
    ...
    {
        return {0,
                {get_begin(std::as_const(std::get< Is >(tup_)))...},
    ...
    private:
        ...
        void next_sub_iterable()
        {
            if (top_level_iter_ != top_level_end_)
            {
                ...
                sub_iter_p_ = std::make_optional< SubIter >(
                    get_begin(sub_iterable_.get()));
                sub_end_p_ =
                    std::make_optional< SubIter >(get_end(sub_iterable_.get()));
            }
            else
            {
                sub_iter_p_.reset();
