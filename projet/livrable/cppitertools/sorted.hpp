...
namespace iter
{
    namespace impl
    {
        ...
        class SortedView;
        using SortedFn =
            IterToolFnOptionalBindSecond< SortedView, std::less<> >;
    }  //namespace impl
...
private:
    ...
    public:
            ...
            container_(std::forward< ContainerT >(container))
        {
            ...
            std::sort(get_begin(sorted_iters_.get()),
                      get_end(sorted_iters_.get()),
    ...
    class SortedItersHolder< ContainerT,
                             std::void_t< decltype(std::begin(
                                 std::declval< AsConst< ContainerT >& >())) > >
    {
    public:
        using IterIterWrap =
            IterIterWrapper< std::vector< iterator_type< ContainerT > > >;
        ...
        using ConstIterIterWrap = IterIterWrapper<
            std::vector< iterator_type< AsConst< ContainerT > > > >;
    ...
    private:
        ...
        void populate_sorted_iters()
        {
            ...
            std::sort(get_begin(sorted_iters_.get()),
                      get_end(sorted_iters_.get()),
                      [this](iterator_type< ContainerT > it1,
        ...
        void populate_const_sorted_iters() const
        {
            ...
            for (auto iter = get_begin(std::as_const(container_));
                 iter != get_end(std::as_const(container_));
                 ++iter)
            {
    ...
    SortedView(Container&& container, CompareFunc compare_func) :
        sorted_iters_holder_ {std::forward< Container >(container),
                              std::move(compare_func)}
    { }
