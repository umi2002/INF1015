...
private:
    ...
    using StarIterDeref = std::remove_reference_t< decltype(std::apply(
        func_, std::declval< iterator_deref< Container > >())) >;

    StarMapper(Func f, Container&& c) :
        func_(std::move(f)),
        container_(std::forward< Container >(c))
    { }
...
private:
    ...
    public:
        ...
        static auto get_and_call_with_tuple(Func& f, TupTypeT& t)
            -> decltype(std::apply(f, std::get< Idx >(t)))
        {  //TODO: Remove duplicated expression in decltype, using
           //decltype(auto) as return type, when all compilers correctly deduce
...
public:
    ...
    auto operator()(Func func, Seq&& sequence) const
    {
        ...
        {
            return StarMapper< Func, Seq > {std::move(func),
                                            std::forward< Seq >(sequence)};
        }
    }
