...
private:
    ...
    friend Compressed iter::compress< Container, Selector >(Container&&,
                                                            Selector&&);
...
public:
    ...
    {
        return {get_begin(container_),
                get_end(container_),
                get_begin(selectors_),
                get_end(selectors_)};
    }

    Iterator< Container, Selector > end()
    {
        return {get_end(container_),
                get_end(container_),
                get_end(selectors_),
                get_end(selectors_)};
    }
...
template< typename Container, typename Selector >
iter::impl::Compressed< Container, Selector >
iter::compress(Container&& container_, Selector&& selectors_)
{
    return {std::forward< Container >(container_),
            std::forward< Selector >(selectors_)};
}
